...
function saveNightEntry() {
  ...
  renderStickyNote(); renderPastJournal(); renderEquanimityBanner();
}
...
function renderEquanimityBanner() {
  const flag=getEquanimityFlag();
  const els=[document.getElementById("equanimityBanner"), document.getElementById("dashEquanimityBanner")].filter(Boolean);
  els.forEach(el=>{
    if (!flag) { el.classList.add("hidden"); return; }
    el.classList.remove("hidden");
    el.innerHTML = `⚠ Same weight showing up two nights running. Per the protocol: name it specifically, then decide — is it a task (schedule it now) or a feeling (say it to one person, or sit with it a few minutes)? If it's still there tomorrow, tomorrow's only requirement is the Minimum Viable Day, not the full chain.`;
  });
}
